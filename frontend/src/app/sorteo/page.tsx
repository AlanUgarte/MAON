...
// Landing pública del sorteo — identidad propia (negro + rojo neón), independiente del
// tema del CRM, mismo criterio que /insumos y /vyno: colores hardcodeados acá adentro.
...
import {
  Zap, ShoppingCart, Trophy, Copy, Check, X, Upload, Search, Loader2, ChevronLeft, ChevronRight,
} from 'lucide-react';
  ...
  return (
    <div style={{ background: BLACK, color: '#f2f2f5', minHeight: '100vh' }}>
      <header
        ...
        style={{ background: 'rgba(10,10,18,.92)', borderBottom: `1px solid ${LINE}` }}
      >
        <div className="mx-auto flex h-[62px] max-w-[1180px] items-center justify-between px-4">
          <a href="#inicio" className="flex items-center gap-2 font-extrabold tracking-tight" style={{ color: RED }}>
            <Zap className="h-5 w-5" fill="currentColor" /> SORTEO
          </a>
        ...
        <section id="inicio" className="grid gap-10 py-12 lg:grid-cols-2">
          <div>
            ...
            <h1
              ...
              style={{ color: RED, textShadow: '0 0 18px rgba(239,30,70,.55)' }}
            >{data.title}</h1>
          </div>
          ...
          <div>
            ...
            <Panel className="text-center">
              ...
              <div className="relative flex h-[26px] items-center overflow-hidden rounded-full" style={{ background: '#2b2b38' }}>
                <span
                  className="absolute inset-y-0 left-0 rounded-full transition-all duration-700"
                  style={{ width: `${Math.max(data.percentSold, 2)}%`, background: 'linear-gradient(90deg,#ef1e46,#ff7a3d)' }}
                />
              ...
              <Panel key={p.id} highlighted={p.isPopular}>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <small className="font-bold" style={{ color: RED }}>{p.chances} Chances por la {data.prize}</small>
                  </div>
                  <div className="text-right">
                    <span className="mb-2 block text-[26px] font-black" style={{ color: RED }}>{money(p.price)}</span>
                    <button
                      onClick={() => setBuying(p)}
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: highlighted ? '0 0 30px rgba(239,30,70,.35)' : '0 0 22px rgba(239,30,70,.18)',
      }}
    >{children}</div>
  );
}
        ...
        <div className="mt-3.5 flex justify-center gap-2">
          {images.map((_, j) => (
            <button
              ...
              style={{ background: j === i ? RED : '#3a3a4a' }}
            />
          ))}
        </div>
      )}
...
function BuyModal({ pkg, data, onClose }: { pkg: Pkg; data: PublicSorteo; onClose: () => void }) {
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-[430px] rounded-2xl p-6" style={{ background: '#101018', border: `1px solid ${LINE}` }}>
        ...
        {done ? (
          <div className="text-center">
            ...
            <h3 className="text-[24px] font-black" style={{ color: '#22c55e' }}>¡Listo!</h3>
          ...
          <>
            ...
            <div className="my-4 rounded-[10px] p-3.5 text-center" style={{ background: '#1c1c26' }}>
              <div className="text-[12px]" style={{ color: 'rgba(255,255,255,.55)' }}>Total a transferir</div>
              <div className="text-[34px] font-black" style={{ color: RED }}>{money(pkg.price)}</div>
            ...
            <div className="mb-4 rounded-[10px] p-3.5" style={{ border: `1px solid ${LINE}` }}>
              ...
              <div className="flex gap-2.5">
                <code
                  ...
                  style={{ background: '#1c1c26', border: `1px solid ${LINE}`, color: RED }}
            ...
            {error && <p className="mt-2.5 text-[13.5px]" style={{ color: '#ff8095' }}>{error}</p>}
...
function Field({ placeholder, value, onChange, type = 'text' }: { placeholder: string; value: string; onChange: (v: string) => void; type?: string }) {
  return (
    <input
      ...
      style={{ background: '#1c1c26', border: `1px solid ${LINE}`, color: '#f2f2f5' }}
    />
  );
}
              ...
              <>
                ...
                <div className="mt-2 flex flex-wrap gap-2.5">
                  {o.numbers.map((n: number) => (
                    <span key={n} className="rounded-md px-4 py-2 text-[18px] font-extrabold" style={{ background: '#f5a623' }}>{n}</span>
                  ))}
                </div>
              </>
            ) : (

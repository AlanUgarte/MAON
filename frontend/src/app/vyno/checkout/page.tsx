...
export default function VynoCheckoutPage() {
  const router = useRouter();
  const { products } = useVynoProductsPublic();
  ...
  const submit = async () => {
    ...
    setSending(true);
    try {
      const res = await api.createVynoOrder({
        ...form,
        items: lines.map((l) => ({ productId: l.productId, quantity: l.quantity })),
      });
      cart.clear();
      router.push(`/vyno/pedido/${res.id}`);
    } catch (err: any) {
      setSending(false);
    ...
    <div style={{ background: CREAM }} className="min-h-screen font-sans">
      <header className="border-b" style={{ background: BLACK, borderColor: 'rgba(255,255,255,0.08)' }}>
        <div className="mx-auto flex h-16 max-w-[900px] items-center gap-4 px-4 sm:px-8">
          <Link href="/vyno/carrito" className="text-white/70 transition hover:text-white"><ArrowLeft className="h-5 w-5" /></Link>
            ...
            <div className="h-fit space-y-4">
              <div className="rounded-sm border bg-white p-5" style={{ borderColor: 'rgba(0,0,0,0.08)' }}>
                ...
                <div className="mt-3 space-y-2">
                  {lines.map((l) => (
                    <div key={l.productId} className="flex justify-between text-[12.5px] text-neutral-600">
                      <span className="truncate pr-2">{l.quantity}× {l.product!.name}</span>

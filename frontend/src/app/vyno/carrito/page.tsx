...
export default function VynoCartPage() {
    ...
    <div style={{ background: CREAM }} className="min-h-screen font-sans">
      <header className="border-b" style={{ background: BLACK, borderColor: 'rgba(255,255,255,0.08)' }}>
        <div className="mx-auto flex h-16 max-w-[900px] items-center gap-4 px-4 sm:px-8">
          <Link href="/vyno" className="text-white/70 transition hover:text-white"><ArrowLeft className="h-5 w-5" /></Link>
          <span className="text-xl font-bold tracking-[0.15em] text-white" style={{ fontFamily: 'var(--font-vyno-serif)' }}>VYNO</span>
              ...
              {lines.map((l) => (
                <div key={l.productId} className="flex items-center gap-4 rounded-sm border bg-white p-4" style={{ borderColor: 'rgba(0,0,0,0.08)' }}>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-0.5 text-[13px] text-neutral-500">{money(l.product!.price)} c/u</div>
                    <div className="mt-2 flex items-center gap-2.5">
                      <button onClick={() => cart.add(l.productId, -1)} className="flex h-8 w-8 items-center justify-center rounded-sm border" style={{ borderColor: 'rgba(0,0,0,0.15)' }}><Minus className="h-3.5 w-3.5" /></button>
                      <span className="w-6 text-center text-[13px] font-bold">{l.quantity}</span>
                      <button onClick={() => cart.add(l.productId, 1)} className="flex h-8 w-8 items-center justify-center rounded-sm border" style={{ borderColor: 'rgba(0,0,0,0.15)' }}><Plus className="h-3.5 w-3.5" /></button>
                    </div>
                  </div>
                  <div className="flex flex-col items-end gap-2">
                    <span className="text-[14px] font-extrabold text-neutral-900">{money(l.product!.price * l.quantity)}</span>
                    <button onClick={() => cart.remove(l.productId)} aria-label="Quitar" className="text-neutral-400 hover:text-rose-600"><Trash2 className="h-4 w-4" /></button>
                  </div>
                </div>
              ))}
              <Link href="/vyno" className="inline-block text-[12.5px] font-semibold underline" style={{ color: '#8A6B2E' }}>← Seguir viendo el producto</Link>
            </div>

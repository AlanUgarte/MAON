...
import { useVynoOrders, type VynoOrder, type VynoOrderStatus } from '@/lib/vyno-orders-store';
import { useVynoProducts } from '@/lib/vyno-products-store';
import { useVynoSettings, type VynoSettings } from '@/lib/vyno-settings-store';

const money = (n: number) => '$' + Math.round(n).toLocaleString('es-AR');
const fmtDate = (d: string) => new Date(d).toLocaleString('es-AR');

const STATUS_LABEL: Record<VynoOrderStatus, string> = {
...
const STATUS_TONE: Record<VynoOrderStatus, 'primary' | 'emerald' | 'amber' | 'rose' | 'sky'> = {
...
export default function VynoConfigPage() {
  const [tab, setTab] = useState<TabKey>('pedidos');
  const orders = useVynoOrders();
  const products = useVynoProducts();
  const vynoSettings = useVynoSettings();

  return (
    <>
      <Topbar title="VYNO" subtitle="Tienda e-commerce — pedidos, producto y configuración" />
      <main className="flex-1 space-y-5 p-5 lg:p-7">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <a href="/vyno" target="_blank" rel="noopener noreferrer" className="flex h-9 items-center gap-1.5 rounded-[10px] border border-line/15 px-3 text-[13px] font-semibold text-muted hover:bg-surface-2 hover:text-content">
        ...
        {tab === 'config' && <ConfigTab settings={vynoSettings.settings} onSave={vynoSettings.save} />}
      </main>
    </>
  );
}
...
function PedidosTab({ orders }: { orders: ReturnType<typeof useVynoOrders> }) {
...
function OrderDetailModal({ order, onClose, onApprove, onSetStatus }: {
  ...
  onSetStatus: (id: string, status: VynoOrderStatus, extra?: { trackingNumber?: string; carrier?: string }) => Promise<void>;
}) {
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto text-[13px]">
          ...
          <div className="space-y-1">
            {order.items.map((it, i) => (
              <div key={i} className="flex justify-between"><span>{it.quantity}× {it.productName}</span><span className="font-semibold">{money(it.subtotal)}</span></div>
            ))}
          ...
          {order.paymentProof ? (
            <div className="rounded-xl border border-line/10 p-3">
              ...
              {order.paymentProof.operationNumber && <div className="mt-1 text-muted">N° operación: {order.paymentProof.operationNumber}</div>}
              {order.paymentProof.holderName && <div className="text-muted">Titular: {order.paymentProof.holderName}</div>}
              {order.paymentProof.comment && <div className="mt-1 text-muted">"{order.paymentProof.comment}"</div>}
            </div>
          ) : (
...
function ProductoTab({ products }: { products: ReturnType<typeof useVynoProducts> }) {
  ...
  if (!product || !current) return <div className="rounded-2xl border border-dashed border-line/15 p-10 text-center text-muted">Todavía no hay un producto VYNO cargado en la base.</div>;
...
function ConfigTab({ settings, onSave }: { settings: VynoSettings; onSave: (s: VynoSettings) => void }) {
  const [form, setForm] = useState(settings);
  const [saved, setSaved] = useState(false);
  const set = <K extends keyof VynoSettings>(k: K, v: VynoSettings[K]) => setForm((f) => ({ ...f, [k]: v }));

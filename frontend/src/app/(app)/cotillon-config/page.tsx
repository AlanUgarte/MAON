'use client';
// Config de FastCotillón (/cotillon) — mismo patrón que tienda-config, filtrado a la
// categoría "Cotillon" del catálogo real. Reemplaza a lo que era Estufa (test) en el nav.
...
import { useCotillonSettings, type CotillonSettings, type ProductPromo } from '@/lib/cotillon-settings-store';
...
// Mismo sku fijo que en /cotillon: matchea contra el catálogo real para saber qué
// pedidos de /sales son de esta línea (cotillón) y no de la tienda mayorista grande.
const isCotillonSku = (sku: string, skus: Set<string>) => skus.has(sku);

export default function CotillonConfigPage() {
  const router = useRouter();
  const { settings, save } = useCotillonSettings();
  ...
  const PRODUCT_ROWS = useMemo(() => fullCatalog.filter((p) => p.category === 'Cotillon'), [fullCatalog]);
  const cotillonSkus = useMemo(() => new Set(PRODUCT_ROWS.map((p) => p.sku)), [PRODUCT_ROWS]);
  const orders = useMemo(() => allOrders.filter((o) => o.items.some((it) => isCotillonSku(it.sku, cotillonSkus))), [allOrders, cotillonSkus]);
  const [form, setForm] = useState<CotillonSettings>(settings);
  ...
  const set = <K extends keyof CotillonSettings>(key: K, value: CotillonSettings[K]) =>
  ...
  const setPromo = (id: string, patch: Partial<ProductPromo>) => {
  ...
  return (
    <>
      <Topbar title="FastCotillón" subtitle="Configurá el banner, el contenido, los productos y revisá los pedidos de FastCotillón" />
      <main className="flex-1 space-y-5 p-5 lg:p-7">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <a href="/cotillon" target="_blank" rel="noreferrer">
        ...
        {tab === 'productos' && (
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><Package className="h-4 w-4 text-primary" /> Productos de cotillón</CardTitle>
              <div className="flex items-center gap-2">
            ...
            <Card>
              ...
              <CardContent>
                {invoicedOrders.length === 0 ? (
                  <div className="rounded-xl border border-dashed border-line/15 p-6 text-center text-[13px] text-muted">
                    Todavía no facturaste ningún pedido de FastCotillón.
                  </div>
                ) : (
                  <div className="space-y-3">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setShowImport(false)}>
          <div className="card flex max-h-[85vh] w-full max-w-lg flex-col p-5" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="mt-1.5 text-[12.5px] text-muted">
              Una línea por producto: <code>SKU | 21x20</code> (2x1, 3x2...), <code>SKU | 10+1</code> (bonificación, cada 10 llevás 1 gratis), <code>SKU | 15%</code>, <code>SKU | NUEVO</code>, o lo mismo con el nombre del producto en vez del SKU. El SKU matchea exacto; el nombre matchea aproximado.
            </p>
            <textarea
              rows={10}
              value={importText}

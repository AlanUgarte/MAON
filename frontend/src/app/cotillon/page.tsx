...
// FastCotillón: catálogo de cotillón (categoría "Cotillon" del catálogo mayorista real),
// antes vivía en un sitio estático aparte (fastcombos.github.io / FastCombos). Mismo patrón
// que /tienda pero filtrado a esa categoría y con su propia marca/config (cotillon-settings).
import { useEffect, useMemo, useState } from 'react';
import { Search, ShoppingCart, X, Plus, Minus, Trash2, MessageCircle, PartyPopper, Sparkles, Truck, ShieldCheck, PackageCheck, SlidersHorizontal, Clock, ChevronLeft, ChevronRight } from 'lucide-react';
...
import { useCotillonSettings } from '@/lib/cotillon-settings-store';
import { useTiendaOrders } from '@/lib/tienda-orders-store';

const BRAND = '#5B2A8C';     // violeta FastCotillón
const BRAND_DARK = '#2D1547';
const BRAND_LIGHT = '#7C4DAE';
const BRAND_SOFT = '#F5EEFA';
...
function CotillonInner() {
  ...
  const { settings } = useCotillonSettings();
  ...
  const catalog = useMemo(() => cotillonOnly.filter((p) => !settings.hiddenProductIds.includes(p.id)), [cotillonOnly, settings.hiddenProductIds]);
  ...
  // "Línea" = sub-categoría del proveedor (ej. "GLOBOS/PIÑATAS", "TOPPER PARA TORTA") —
  // el filtro que tenía el sitio FastCombos original, acá sale del campo `line` del catálogo.
  const lineas = useMemo(() => [...new Set(catalog.map((p) => p.line).filter(Boolean))].sort() as string[], [catalog]);
  ...
  const goHome = () => {
    setSearch(''); setLinea(''); setView(''); setBrand(''); setBrandQuery('');
  ...
  const buildOrderText = () => {
    ...
    return `¡Hola! Quiero hacer este pedido en *FastCotillón*:\n\n${lines}\n\n${envio}\n*Total: ${money(subtotal)}*\n\nNombre: ${form.name}\nTeléfono: ${form.phone}\n\nEl pedido se despacha entre 24 y 48 hs.`;
  };
  ...
  const sendOrder = async () => {
    ...
    const client = existing ?? addClient({
      ...
      lastInboundAt: new Date().toISOString(), unread: 1, summary: 'Pedido armado desde FastCotillón.',
  ...
  if (!settings.storeOpen) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-3 px-4 text-center" style={{ background: '#FAFAF7' }}>
        ...
        <h1 className="font-display text-xl font-extrabold" style={{ color: BRAND }}>FastCotillón</h1>
      ...
      <header className="sticky top-0 z-30 border-b border-black/[0.06] bg-white/90 px-4 py-3 shadow-[0_1px_0_0_rgba(0,0,0,0.04)] backdrop-blur">
        <div className="mx-auto flex max-w-[1600px] items-center gap-3 sm:gap-4">
          <button
            ...
            aria-label="Volver al inicio de FastCotillón"
          >
            ...
            <div className="hidden leading-none sm:block">
              <div className="font-display text-lg font-extrabold tracking-tight" style={{ color: BRAND }}>Fast<span style={{ color: ACCENT }}>Cotillón</span></div>
              <div className="text-[10px] font-medium uppercase tracking-widest text-neutral-400">Cotillón al por mayor</div>
            </div>
          </button>
        ...
        {/* Líneas (sub-categorías de cotillón, ej. Globos/Piñatas, Topper para torta) */}
        <div className="mx-auto mt-1.5 flex max-w-[1600px] gap-1.5 overflow-x-auto pb-0.5">
          <button
            ...
            style={!linea ? { background: BRAND, color: '#fff' } : { background: '#F1F1EC', color: '#666' }}
          >
            Todas
          </button>
          {lineas.map((l) => (
            <button
              ...
              onClick={() => setLinea(l)}
              className="shrink-0 rounded-full px-3.5 py-1.5 text-[12px] font-semibold transition"
              style={linea === l ? { background: BRAND, color: '#fff' } : { background: '#F1F1EC', color: '#666' }}
            >
              🎉 {l}
            </button>
          ))}
      ...
      {activeCarousel.length > 0 ? (
        ...
        <div
          ...
          style={settings.heroImageUrl
            ? { backgroundImage: `linear-gradient(rgba(45,21,71,.55), rgba(45,21,71,.55)), url(${settings.heroImageUrl})` }
        ...
        <section className="flex-1">
          <div className="mb-3 flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-3">
              ...
              {(linea || brand || search || priceMin || priceMax) && (
                <button onClick={() => { setLinea(''); setBrand(''); setSearch(''); setPriceMin(''); setPriceMax(''); setBrandQuery(''); }} className="text-[12px] font-semibold" style={{ color: ACCENT }}>
                  Limpiar filtros
                </button>
              )}
            </div>
              ...
              return (
                <div key={p.id} className="group flex flex-col rounded-2xl border border-black/5 bg-white p-3 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
                  <div className="relative flex items-center justify-center overflow-hidden rounded-xl p-3" style={{ background: BRAND_SOFT }}>
                    <span className="absolute left-2 top-2 z-10 rounded-md bg-white/90 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide" style={{ color: BRAND }}>Por unidad</span>
                    {promo?.isNew && (
...
export default function CotillonPage() {
  return <CotillonInner />;
}

...
@ApiTags('Vyno')
@Controller('vyno-orders')
export class VynoOrdersController {
  constructor(private readonly orders: VynoOrdersService) {}

  // Pública: checkout sin login. Límite propio (8/min por IP), mismo criterio que
  // /sales/storefront y /supremas-sales/storefront — es el único endpoint de
  // escritura sin login acá.
  @UseGuards(ThrottlerGuard) @Throttle({ default: { ttl: 60_000, limit: 8 } })
  @Post() create(@Body() dto: CreateVynoOrderDto) { return this.orders.create(dto); }
  ...
  @ApiBearerAuth() @UseGuards(JwtAuthGuard, RolesGuard) @Roles('ADMINISTRADOR', 'SUPERVISOR')
  @Patch(':id/status') setStatus(@Param('id') id: string, @Body() dto: SetVynoOrderStatusDto) {

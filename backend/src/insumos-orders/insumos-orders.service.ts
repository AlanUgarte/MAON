...
import { InsumosSettingsService } from '../insumos-settings/insumos-settings.service';
...
@Injectable()
export class InsumosOrdersService {
  constructor(
    private readonly prisma: PrismaService,
    private readonly settings: InsumosSettingsService,
  ) {}

  findAll(status?: string) {
    return this.prisma.insumosOrder.findMany({
  ...
  async findOne(id: string) {
    const order = await this.prisma.insumosOrder.findUnique({ where: { id }, include: ORDER_INCLUDE });
    if (!order) throw new NotFoundException('Pedido no encontrado');
    return order;
  }

  /** Vista pública de un pedido (confirmación/seguimiento) — el id (cuid, no
   * adivinable) funciona como token de acceso, mismo criterio que VYNO. Sin datos
   * sensibles del cliente más allá de lo que el propio cliente ya cargó. */
  async findPublic(id: string) {
    ...
    return {
      ...
      items: order.items.map((i) => ({ name: i.product.name, size: i.size, quantity: i.quantity, unitPrice: i.unitPrice, subtotal: i.subtotal })),
   ...
   * descuenta stock. Arranca en PAGO_PENDIENTE porque el único medio es transferencia. */
  async create(dto: CreateInsumosOrderDto) {
    if (!dto.items?.length) throw new BadRequestException('El carrito está vacío');

    const productIds = dto.items.map((i) => i.productId);
    const products = await this.prisma.insumosProduct.findMany({ where: { id: { in: productIds }, isActive: true } });
    const byId = new Map(products.map((p) => [p.id, p]));

    let subtotal = 0;
    const itemsData: { productId: string; quantity: number; size: string; unitPrice: number; subtotal: number }[] = [];
    for (const it of dto.items) {
      ...
      itemsData.push({ productId: prod.id, quantity: it.quantity, size: it.size, unitPrice, subtotal: lineSubtotal });
    }
    ...
    const count = await this.prisma.insumosOrder.count();
    const orderNumber = `INS-${String(count + 1).padStart(6, '0')}`;

    const order = await this.prisma.$transaction(async (tx) => {
      const created = await tx.insumosOrder.create({
        data: {
          orderNumber,
          clientId: client.id,
      ...
      for (const it of itemsData) {
        await tx.insumosProduct.update({ where: { id: it.productId }, data: { stock: { decrement: it.quantity } } });
      }
      return created;
    });
  ...
  async reportPayment(orderId: string, dto: ReportPaymentDto) {
    ...
    await this.prisma.insumosPaymentProof.upsert({
      where: { orderId },
      update: dto,
      create: { orderId, ...dto },
    });
    return this.prisma.insumosOrder.update({
      where: { id: orderId },
  ...
  async setStatus(orderId: string, dto: SetInsumosOrderStatusDto) {

...
export class InsumosOrderItemDto {
  @ApiProperty() @IsString() productId: string;
  @ApiProperty() @IsInt() @Min(1) @Max(50) quantity: number;
  /// Medida elegida (largo en metros, ej. "3.50") — obligatoria para este producto.
  @ApiProperty() @IsString() @MaxLength(20) size: string;
}

/** Pedido armado en la tienda pública de Insumos Carnicería (sin login): identifica
 * al cliente por teléfono/email, igual criterio que el resto de los storefronts. El
 * precio NUNCA lo manda el cliente — el server lo toma siempre de InsumosProduct.price. */
export class CreateInsumosOrderDto {
  ...
  @ApiProperty({ type: [InsumosOrderItemDto] })
  @IsArray() @ValidateNested({ each: true }) @Type(() => InsumosOrderItemDto)
  items: InsumosOrderItemDto[];
}
